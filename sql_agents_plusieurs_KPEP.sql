--===========================================
-- Sélection finale : Compte le nombre de personnes ayant plusieurs KPEP.
--===========================================
WITH ass AS (
    --===========================================
    -- CTE : ass
...
ens_ctr AS (
    ...
    -- Cette CTE est définie mais n'est pas utilisée dans la requête principale.
    --===========================================
    SELECT
...
--===========================================
-- Sélection finale : Compte les personnes ayant plusieurs KPEP.
--===========================================
...
FROM (
    ...
    FROM
        ass
    -- La CTE 'ens_ctr' n'est pas utilisée ici, contrairement à la requête précédente.
    INNER JOIN contrats ON (ass.refunqctr = contrats.noctr1dv)
    INNER JOIN rup ON (ass.refsrcper || '' = rup.num_personne AND TRIM(rup.compagny_code) = TRIM(contrats.codsoc))
) t
